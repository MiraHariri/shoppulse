 ...
 */
function errorResponse(statusCode: number, error: string, field?: string, retryable?: boolean): LambdaResponse {
 ...
 */
async function validateTenantAdmin(userId: string, tenantId: string): Promise<void> {
  const result = await query<{ is_tenant_admin: boolean }>(
    'SELECT is_tenant_admin FROM users WHERE user_id = $1 AND tenant_id = $2 AND status = $3',
 ...
 */
async function validateUserTenant(userId: string, tenantId: string): Promise<boolean> {
 ...
 * POST /users
 */
export async function createUser(event: APIGatewayEvent): Promise<LambdaResponse> {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const userIdResult = await client.query<{ user_id: string }>(
        `SELECT 'U' || LPAD(COALESCE(MAX(SUBSTRING(user_id FROM 2)::INTEGER), 0) + 1::TEXT, 3, '0') AS user_id 
      ...
      return { userId, email: requestData.email, role: requestData.role };
    });
    ...
    console.log(`User created: ${result.userId} in tenant ${context.tenantId}`);
 ...
 * GET /users
 */
export async function listUsers(event: APIGatewayEvent): Promise<LambdaResponse> {
  try {
    const context = getRequestContext(event);
    ...
    return errorResponse(500, 'Failed to list users', undefined, true);
  }
}
...
export async function getUser(event: APIGatewayEvent): Promise<LambdaResponse> {
  try {
    ...
    if (!isValidTenant) {
      return errorResponse(403, 'Access denied: resource belongs to different tenant');
    }
 ...
 */
export async function updateUserRole(event: APIGatewayEvent): Promise<LambdaResponse> {
  try {
    const context = getRequestContext(event);
    const userId = event.pathParameters?.userId;
    ...
    await withTransaction(async (client) => {
      ...
      const userResult = await client.query<{ cognito_user_id: string; email: string }>(
    ...
    console.log(`User role updated: ${userId} to ${requestData.role} in tenant ${context.tenantId}`);
    return successResponse(200, { success: true, userId, role: requestData.role });
  } catch (error: any) {
 ...
 * DELETE /users/{userId}
 */
export async function deleteUser(event: APIGatewayEvent): Promise<LambdaResponse> {
  try {
    const context = getRequestContext(event);
    const userId = event.pathParameters?.userId;
